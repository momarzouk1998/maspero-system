    ...
    <div className="space-y-6">
      ...
      <div className="glass-panel p-6 rounded-3xl border border-slate-200 space-y-4">
        <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
          ...
          <span>سجل الحركات المسجلة مؤخراً</span>
        </h3>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-right text-sm text-slate-700">
            <thead className="bg-slate-100 text-slate-700 text-xs font-semibold uppercase border-b border-slate-200">
              <tr>
                <th className="px-4 py-3">النوع / التوعية</th>
                <th className="px-4 py-3">التصنيف</th>
                <th className="px-4 py-3">القيمة / الساعات</th>
            ...
            <tbody className="divide-y divide-slate-200">
              {loading ? (
                <tr>
                  <td colSpan={7} className="text-center py-12 text-slate-500">جاري التحميل...</td>
                </tr>
              ) : activeTab === 'financial' ? (
                expensesList.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="text-center py-12 text-slate-500">لا توجد معاملات مالية مسجلة</td>
                  </tr>
                ) : (
                  expensesList.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-4 py-3 font-bold text-slate-900">{item.main_type}</td>
                      ...
                      <td className="px-4 py-3 text-xs text-slate-700">{item.employee_name || '-'}</td>
                      ...
                      {currentUser?.role === 'manager' && (
                        <td className="px-4 py-3 text-center">
                          <button
                            onClick={async () => {
                              if (!confirm('هل أنت تأكد من رغبتك في حذف هذه المعاملة المالية؟')) return;
                ...
                hrList.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="text-center py-12 text-slate-500">لا توجد طلبات حوافز أو إجازات مسجلة</td>
                  </tr>
                ) : (
                  hrList.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-4 py-3 font-bold text-indigo-700">{item.hr_items}</td>
                      <td className="px-4 py-3 text-xs text-slate-600">طلب بالساعات</td>
                      <td className="px-4 py-3 font-bold font-mono text-indigo-900">
                        {Number(item.hours).toFixed(2)} ساعة
                      </td>
                      <td className="px-4 py-3 text-xs text-slate-700">{item.employee_name || item.e_hr_name || '-'}</td>
                      <td className="px-4 py-3 text-xs text-slate-600 font-mono">
                        {new Date(item.timestamp || item.date).toLocaleDateString('ar-EG')}
                      </td>
                      <td className="px-4 py-3 text-xs text-slate-500">{item.notes || '-'}</td>
                        ...
                        <td className="px-4 py-3 text-center">
                          <button
                            onClick={async () => {
                              ...
                              await fetch(`/api/hr?id=${item.id}`, { method: 'DELETE' });
                              fetchData();
                            }}
                            className="p-1.5 bg-red-100 hover:bg-red-200 text-red-700 rounded-lg border border-red-200 transition-colors"
                            title="حذف الطلب"
