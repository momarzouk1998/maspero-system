      ...
      <div className="glass-panel px-4 py-3.5 rounded-2xl border border-slate-200 space-y-2.5">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-base md:text-xl font-bold text-slate-900 flex items-center gap-2">
            <Printer className="w-5 h-5 md:w-6 md:h-6 text-blue-600 shrink-0" />
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1 min-w-0">
            ...
            <input
              ...
              className="w-full pl-4 pr-10 py-2.5 bg-white border border-slate-300 rounded-xl text-slate-900 text-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200"
            />
          </div>
          <button
            onClick={() => setIsFilterOpen(true)}
            className={`py-2.5 px-3.5 rounded-xl text-xs font-bold flex items-center gap-1.5 border transition-all shrink-0 active:scale-95 ${
              hasActiveFilters
                  ...
                  <div key={m.month} className="space-y-1">
                    ...
                    >
                      ...
                      <span className={`font-mono font-bold text-[11px] px-1.5 py-0.5 rounded ${
                        isMonthOnlySelected ? 'bg-white/20 text-white' : 'text-blue-700 bg-blue-50'
                    ...
                    {isMonthExpanded && m.days && (
                      <div className="pr-3 space-y-1 border-r-2 border-blue-200 mr-2">
                        {m.days.map((d: any) => {
                          ...
                          const isDayOnlySelected = selectedDay === d.day && !selectedCategory && !filterServiceName;
                            ...
                            <div key={d.day} className="space-y-1">
                              <div
                                ...
                                className={`p-1.5 rounded-lg border text-[11px] cursor-pointer flex items-center justify-between transition-all ${
                                  isDayOnlySelected
                                    ? 'bg-blue-600 text-white font-extrabold border-blue-600 shadow-md ring-2 ring-blue-400'
                                    : isDayActive
                                    ? 'bg-blue-100 border-blue-400 text-blue-950 font-bold shadow-sm ring-1 ring-blue-300'
                                    : 'bg-white hover:bg-blue-50 border-slate-200 text-slate-700'
                                }`}
                              >
                                <span className="font-bold">📆 {d.day}</span>
                                <span className={`font-mono font-bold px-1 py-0.5 rounded ${
                                  isDayOnlySelected ? 'bg-white/20 text-white' : isDayActive ? 'bg-blue-200 text-blue-900' : 'text-blue-600'
                                    ...
                                    return (
                                      <div
                                        ...
                                        className={`p-1.5 rounded-md border text-[10px] cursor-pointer flex items-center justify-between transition-all ${
                                          isCatSelected
                                            ? 'bg-blue-600 text-white font-extrabold border-blue-600 shadow-md ring-2 ring-blue-400 scale-[1.02]'
                                            : 'bg-slate-50 hover:bg-blue-100/60 border-slate-200 text-slate-700'
                                        }`}
                                      >
                                        <div className="flex items-center gap-1.5">
                                          <span className={`w-2 h-2 rounded-full ${isCatSelected ? 'bg-white shadow-sm' : 'bg-blue-500'}`} />
                                          <span className={isCatSelected ? 'font-black tracking-wide' : 'font-medium'}>🖨️ {c.category}</span>
                                        </div>
                                        <span className={`font-mono font-bold px-1 py-0.5 rounded ${
