...
export async function GET(req: Request) {
  ...
  try {
    ...
    const records = await db.employee_hr.findMany({
      where: userFilter,
      select: {
        id: true,
        hours: true,
    ...
    records.forEach(rec => {
      const amt = Number(rec.hours || 0);
      grandTotal += amt;
      totalCount += 1;

      const dateObj = new Date(rec.timestamp || rec.date || Date.now());
      ...
      const catKey = rec.hr_items || 'سلف وحوافز';
    ...
    console.error('HR history tree error:', e);
